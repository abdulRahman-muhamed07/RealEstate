...
public sealed class AuthService(AppDbContext db, ITokenService tokenService) : IAuthService
{
    ...
    public async Task<Result<AuthResponse>> RegisterAsync(RegisterRequest request, CancellationToken ct)
    {
        ...
        if (request.Role == UserRole.Admin)
            return Result<AuthResponse>.Fail(ErrorCode.Forbidden, "Admin registration is not allowed.");

        if (await db.Users.AnyAsync(x => x.Email == email, ct))
            return Result<AuthResponse>.Fail(ErrorCode.Conflict, "Email already exists.");
        ...
        {
            return Result<AuthResponse>.Fail(ErrorCode.Conflict, "The account could not be created because the email already exists.");
        }
    ...
    public async Task<Result<AuthResponse>> LoginAsync(LoginRequest request, CancellationToken ct)
    {
        ...
        if (user is null)
            return Result<AuthResponse>.Fail(ErrorCode.Unauthorized, "Invalid email or password.");
