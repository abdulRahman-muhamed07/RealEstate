...
public sealed class PropertyService(AppDbContext db, IImageStorage imageStorage) : IPropertyQueryService, IPropertyCommandService
{
    // Query operations
    public async Task<Result<PagedResult<PropertyListItem>>> SearchAsync(PropertyFilterRequest request, CancellationToken ct)
    {
        ...
        var properties = await query.Skip((page - 1) * pageSize).Take(pageSize).Select(MapExpression()).ToListAsync(ct);
        var totalPages = (int)Math.Ceiling(totalCount / (double)pageSize);

        return Result<PagedResult<PropertyListItem>>.Ok(new PagedResult<PropertyListItem>(properties, totalCount, page, pageSize, totalPages));
    }
    ...
    public async Task<Result<PropertyDetails>> GetByIdAsync(int id, CancellationToken ct)
    {
        ...
        var averageRating = await db.Reviews.Where(x => x.PropertyId == id).Select(x => (double?)x.Rating).AverageAsync(ct) ?? 0;
    ...
    public async Task<IReadOnlyList<PropertyListItem>> GetMineAsync(string userId, CancellationToken ct) =>
        await db.Properties.AsNoTracking().Where(x => x.OwnerId == userId).OrderByDescending(x => x.CreatedAt).Select(MapExpression()).ToListAsync(ct);

    public async Task<IReadOnlyList<PropertyListItem>> GetPendingAsync(CancellationToken ct) =>
        await db.Properties.AsNoTracking().Where(x => !x.IsApproved).OrderByDescending(x => x.CreatedAt).Select(MapExpression()).ToListAsync(ct);
    ...
    public async Task<Result<int>> CreateAsync(CreatePropertyRequest request, string userId, CancellationToken ct)
    {
        ...
        {
            Title = request.Title.Trim(), Description = request.Description.Trim(), Price = request.Price, Area = request.Area,
            Bedrooms = request.Bedrooms, Bathrooms = request.Bathrooms, Type = request.Type.Trim().ToLowerInvariant(),
            ListingType = request.ListingType, Location = request.Location.Trim(), CategoryId = request.CategoryId, CityId = request.CityId,
    ...
    private static IQueryable<Property> ApplyFilters(IQueryable<Property> query, PropertyFilterRequest request)
    {
    ...
    private static IQueryable<Property> ApplySorting(IQueryable<Property> query, string sortBy) => sortBy.Trim().ToLowerInvariant() switch
    {
    ...
    {
        if (!await db.Categories.AnyAsync(x => x.Id == request.CategoryId, ct)) return "Category not found.";
        if (request.CityId.HasValue && !await db.Cities.AnyAsync(x => x.Id == request.CityId.Value, ct)) return "City not found.";
        return null;
    }
    ...
    private static void UpdatePropertyValues(Property property, UpdatePropertyRequest request, bool isAdmin)
    {
        ...
        property.Type = request.Type.Trim().ToLowerInvariant(); property.ListingType = request.ListingType; property.Location = request.Location.Trim();
        property.CategoryId = request.CategoryId; property.CityId = request.CityId; property.IsApproved = isAdmin; property.Status = PropertyStatus.Available;
    }
    ...
    private static PropertyListItem Map(Property property) => new(
    ...
    private static System.Linq.Expressions.Expression<Func<Property, PropertyListItem>> MapExpression() => property => new PropertyListItem(
        property.Id, property.Title, property.Price, property.Area, property.Bedrooms, property.Bathrooms, property.Type,
