...
public sealed class PropertyService(AppDbContext db, IImageStorage imageStorage) : IPropertyService
{
    public async Task<Result<PagedResult<PropertyListItem>>> SearchAsync(PropertyFilterRequest request, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Type)) query = query.Where(x => x.Type == request.Type.Trim().ToLowerInvariant());
        ...
        return Result<PagedResult<PropertyListItem>>.Ok(new PagedResult<PropertyListItem>(data, total, page, pageSize, (int)Math.Ceiling(total / (double)pageSize)));
    }

    public async Task<Result<PropertyDetails>> GetByIdAsync(int id, CancellationToken ct)
    {
        var property = await db.Properties.AsNoTracking().Include(x => x.Category).Include(x => x.City).Include(x => x.Owner).Include(x => x.Images).FirstOrDefaultAsync(x => x.Id == id && x.IsApproved, ct);
        ...
        return Result<PropertyDetails>.Ok(new PropertyDetails(Map(property), rating, count));
    }
    ...
    public async Task<Result<int>> CreateAsync(CreatePropertyRequest request, string userId, CancellationToken ct)
    {
        ...
        var property = new Property { Title = request.Title.Trim(), Description = request.Description.Trim(), Price = request.Price, Area = request.Area, Bedrooms = request.Bedrooms, Bathrooms = request.Bathrooms, Type = request.Type.Trim().ToLowerInvariant(), ListingType = request.ListingType, Location = request.Location.Trim(), CategoryId = request.CategoryId, CityId = request.CityId, OwnerId = userId, IsApproved = user.Role == UserRole.Admin };
    ...
    public async Task<Result<bool>> UpdateAsync(int id, UpdatePropertyRequest request, string userId, bool isAdmin, CancellationToken ct)
    {
        ...
        property.Title = request.Title.Trim(); property.Description = request.Description.Trim(); property.Price = request.Price; property.Area = request.Area; property.Bedrooms = request.Bedrooms; property.Bathrooms = request.Bathrooms; property.Type = request.Type.Trim().ToLowerInvariant(); property.ListingType = request.ListingType; property.Location = request.Location.Trim(); property.CategoryId = request.CategoryId; property.CityId = request.CityId; property.IsApproved = isAdmin; property.Status = PropertyStatus.Available;
        await using var transaction = await db.Database.BeginTransactionAsync(ct);
    ...
    public async Task<IReadOnlyList<PropertyListItem>> GetMineAsync(string userId, CancellationToken ct) => await db.Properties.AsNoTracking().Where(x => x.OwnerId == userId).Include(x => x.Category).Include(x => x.City).Include(x => x.Owner).Include(x => x.Images).OrderByDescending(x => x.CreatedAt).Select(MapExpression()).ToListAsync(ct);
    public async Task<IReadOnlyList<PropertyListItem>> GetPendingAsync(CancellationToken ct) => await db.Properties.AsNoTracking().Where(x => !x.IsApproved).Include(x => x.Category).Include(x => x.City).Include(x => x.Owner).Include(x => x.Images).OrderByDescending(x => x.CreatedAt).Select(MapExpression()).ToListAsync(ct);
    public async Task<Result<bool>> ApproveAsync(int id, ApprovePropertyRequest request, CancellationToken ct)
    {
        ...
        if (property is null) return Result<bool>.Fail(ErrorCode.NotFound, "Property not found.");
        if (!request.Approve) db.Properties.Remove(property);
        else { property.IsApproved = true; if (request.ListingType.HasValue) property.ListingType = request.ListingType.Value; property.Status = PropertyStatus.Available; }
        await db.SaveChangesAsync(ct);
        return Result<bool>.Ok(true);
    }
    private static PropertyListItem Map(Property p) => new(p.Id, p.Title, p.Price, p.Area, p.Bedrooms, p.Bathrooms, p.Type, p.ListingType, p.Status, p.Location, p.CreatedAt, p.CategoryId, p.Category?.Name ?? string.Empty, p.CityId, p.City?.Name, p.OwnerId, $"{p.Owner?.FirstName} {p.Owner?.LastName}".Trim(), p.Images.Select(x => x.Url).ToList());
    private static System.Linq.Expressions.Expression<Func<Property, PropertyListItem>> MapExpression() => p => new PropertyListItem(p.Id, p.Title, p.Price, p.Area, p.Bedrooms, p.Bathrooms, p.Type, p.ListingType, p.Status, p.Location, p.CreatedAt, p.CategoryId, p.Category.Name, p.CityId, p.City == null ? null : p.City.Name, p.OwnerId, (p.Owner.FirstName + " " + p.Owner.LastName).Trim(), p.Images.Select(i => i.Url).ToList());
}
